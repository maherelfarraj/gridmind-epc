...
def test_configure_script_skips_conflicting_rename_and_keeps_configuring_domains():
    _VercelStubHandler.calls = []
    server = HTTPServer(("127.0.0.1", 0), _VercelStubHandler)
    ...
    assert ("PATCH", "/v9/projects/gridmind-epc", '{"rootDirectory":"web"}') in _VercelStubHandler.calls
    assert ("PATCH", "/v9/projects/gridmind-epc", '{"name":"gridmindepc"}') in _VercelStubHandler.calls
    assert ("POST", "/v10/projects/gridmind-epc/domains", '{"name":"pvmind.ai"}') in _VercelStubHandler.calls
